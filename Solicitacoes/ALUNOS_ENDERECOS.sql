...
--DROP VIEW VW_ALUNOS_ENDERECOS 
CREATE VIEW VW_ALUNOS_ENDERECOS AS 
SELECT distinct 
...
---------------------------------------------------------
--SELECT * FROM VW_ALUNOS_ENDERECOS

SELECT cd_distrito_mec,cd_municipio,nm_distrito_mec,B.*

FROM D_DISTRITO_MEC A
JOIN D_UNIDADE_ADMINISTRATIVA B ON A.cd_unidade_educacao=B.cd_unidade_administrativa


WHERE CD_MUNICIPIO=3550308 AND dt_cancelamento IS NULL AND cd_distrito_mec<>0
