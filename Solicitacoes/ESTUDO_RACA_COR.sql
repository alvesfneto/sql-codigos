...
COUNT (DISTINCT CASE WHEN (SG_ETAPA IS NOT NULL AND TP_RACA_COR=5) THEN CD_ALUNO ELSE NULL END) AS INDÍGENA,
COUNT (DISTINCT CASE WHEN (SG_ETAPA IS NOT NULL AND TP_RACA_COR=1) THEN CD_ALUNO ELSE NULL END) AS BRANCA,
COUNT (DISTINCT CASE WHEN (SG_ETAPA IS NOT NULL AND TP_RACA_COR=2) THEN CD_ALUNO ELSE NULL END) AS PRETA,
COUNT (DISTINCT CASE WHEN (SG_ETAPA IS NOT NULL AND TP_RACA_COR=3) THEN CD_ALUNO ELSE NULL END) AS PARDA,
COUNT (DISTINCT CASE WHEN (SG_ETAPA IS NOT NULL AND TP_RACA_COR=4) THEN CD_ALUNO ELSE NULL END) AS AMARELA,
COUNT (DISTINCT CASE WHEN (SG_ETAPA IS NOT NULL AND TP_RACA_COR=6) THEN CD_ALUNO ELSE NULL END) AS [NÃO INFORMADA],
COUNT (DISTINCT CASE WHEN (SG_ETAPA IS NOT NULL AND TP_RACA_COR=7) THEN CD_ALUNO ELSE NULL END) AS [RECUSOU INFORMAR]
INTO ##MATRICULAS_RACA_COR
FROM SME_ALUNOS_DIARIO  A
JOIN SME_ESCOLA_DIARIO B ON A.CD_ESCOLA=B.CD_ESCOLA
WHERE SG_ETAPA IS NOT NULL AND B.TP_ESCOLA IN (1,2,3,4,10,11,12,13,16,17,18,20,28,30,31,32,33)
GROUP BY
B.DRE,
B.CD_ESCOLA,
B.SG_TP_ESCOLA,
